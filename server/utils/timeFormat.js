...
export function resolveScheduleTimes(departureTime, arrivalTime, validateTimeRange) {
  const { departureTime: dep, arrivalTime: arr } = normalizeTimeRange(
    departureTime,
    arrivalTime
  )
  const timeError = validateTimeRange(dep, arr)
  if (timeError) {
...
export function sanitizeWorkingHoursInput(value) {
  if (value == null || value === '') return undefined
  const normalized = normalizeWorkingHours(value)
  if (!normalized) {
    const error = new Error('Working hours must be in HH:mm - HH:mm format')
    error.statusCode = 400
    throw error
  }
  if (/off|leave|unavailable/i.test(normalized)) return normalized
